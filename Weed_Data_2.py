...
'''def main():
	...
	#flush (?) system to print before plotting graphs
	sys.stdout.flush()

	overall_plot(all_month_list, all_num_list)
'''

csv_data = pd.read_csv('/Users/Will/Documents/Projects/Coding/Random/Weed_Data/weed_data.csv')
	
def year_column(year):
	return csv_data[year]

print(csv_data)
print(year_column('2011'))
print(csv_data.describe())
plt.plot(csv_data.2011, csv_data.describe(mean))

# Pulls in frequency numbers from csv file and forms a list for the year
def freq_lists(year):
	freq_list = []
	with open("/Users/Will/Documents/Projects/Coding/Random/Weed_Data/weed_data.csv") as weed_csv:
...
all_month_list_of_lists = [date_increment(i) for i in range(2010, datetime.now().year + 1)]
all_month_list = [i for sublist in all_month_list_of_lists for i in sublist]
all_num_list_of_lists = [freq_lists(i) for i in range(2010, datetime.now().year + 1)]
all_num_list = [i for sublist in all_num_list_of_lists for i in sublist]
...
'''def highest_year(data):
	...
	total = 0
	for lst in data:
		yearly_total = sum(lst.values())
		if yearly_total >= total:
			total = yearly_total
			highest_year = year
		year += 1
	return "{year} with {num} times".format(year=highest_year, num=total)'''
...
# Returns info on frequency by season
...
def print_seasonal_trends(data, season):
   ...
   Avgs for season: monthly = {month} | weekly = {week} | days / time = {days}
           Compare: monthly = {month_diff} | weekly = {week_diff} | days / time = {day_diff} \
   '''.format(total=grand_total, season=season, month=monthly_avg(new_dict), week=weekly_avg(new_dict), days=days_per_time(new_dict), month_diff=compare_avg("monthly", new_dict), week_diff=compare_avg("weekly", new_dict), day_diff=compare_avg("daily", new_dict))
...
def compare_seasons():
	...
	print('''Highest season avg = {high}\nLowest season avg = {low} \
		'''.format(high=highest_season, low=lowest_season))


def overall_plot(month, num):
	plt.plot(month, num)
	plt.ylabel("Number of Times")
	plt.xlabel("Month")
	plt.xticks(rotation=60)
	plt.gcf().subplots_adjust(bottom=0.3)
	plt.plot(month, num, "b.")
	plt.show()
